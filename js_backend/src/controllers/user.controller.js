...
const loginUser = asyncHandler( async(req, res) => {
    ...
    if(!user)
    {
        ...
        .json(
            new ApiResponse(401, "Invalid Username or Email")
        )
    }
...
const getWatchHistory = asyncHandler( async(req, res) => {
    ...
    const user = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                foreignField: "_id",
                as: "watchHistory",
                //pipeline to join users and videos owner // nested pipeline
                pipeline: [
                    {
                        $lookup: {
                            from: "users",
                            ...
                            foreignField: "_id",
                            as: "owner",
                            //pipeline to get only selected details of user
                            pipeline: [
                                {
                                    $project: {
                                        fullName: 1,
    ...
    return res
    .status(200)
    .json(new ApiError(200, "watch history fetched successfully", user[0].watchHistory))
})
